import { MotorcycleRegistration, OfficerAssignment, PrintBatchOrder, UserRole, Language, VerificationLog } from '../types';

const KEYS = {
  REGISTRATIONS: 'bd_motor_registrations',
  OFFICERS: 'bd_motor_officer_assignments',
  PRINT_ORDERS: 'bd_motor_print_batch_orders',
  VERIFICATIONS: 'bd_motor_verification_logs',
...
export function getStoredRegistrations(): MotorcycleRegistration[] {
  return getStoredItem<MotorcycleRegistration[]>(KEYS.REGISTRATIONS, []);
}
...
export function getStoredOfficers(): OfficerAssignment[] {
  return getStoredItem<OfficerAssignment[]>(KEYS.OFFICERS, []);
}
...
export function getStoredPrintOrders(): PrintBatchOrder[] {
  return getStoredItem<PrintBatchOrder[]>(KEYS.PRINT_ORDERS, []);
}
...
export function getStoredVerificationLogs(): VerificationLog[] {
  const logs = getStoredItem<VerificationLog[]>(KEYS.VERIFICATIONS, []);
  return logs;
}

export function saveVerificationLogs(logs: VerificationLog[]): void {
  setStoredItem(KEYS.VERIFICATIONS, logs);
}
