...
export const validateSingleName = (name: string, fieldLabel: string, isAmharic: boolean): ValidationResult => {
  ...
  if (trimmed.length < 2) {
    return {
      isValid: false,
      message: isAmharic ? `${fieldLabel} ቢያንስ 2 ቁምፊዎች መሆን አለበት!` : `${fieldLabel} must be at least 2 characters!`,
    };
  }
  const nameRegex = /^[A-Za-z\s\u1200-\u137F'-]+$/;
  if (!nameRegex.test(trimmed)) {
    return {
      isValid: false,
      message: isAmharic
        ? `${fieldLabel} ውስጥ ልዩ ምልክቶች ወይም ቁጥሮች አይፈቀዱም!`
        : `Special characters or numbers are not allowed in ${fieldLabel}!`,
    };
  }
  return { isValid: true, message: '' };
};
