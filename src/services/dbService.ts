...
export function mergeById<T extends { id: string }>(incoming: T[], existing: T[]): T[] {
...
export function loadStateFromLocalStorage(): boolean {
  ...
  try {
    ...
    if (Array.isArray(parsed.registrations) && parsed.registrations.length > 0) {
      inMemory.registrations = mergeById(parsed.registrations, inMemory.registrations);
      notifyRegistrations();
      loaded = true;
    }
    if (Array.isArray(parsed.officers) && parsed.officers.length > 0) {
      inMemory.officers = mergeById(parsed.officers, inMemory.officers);
      notifyOfficers();
      loaded = true;
    }
    if (Array.isArray(parsed.printOrders) && parsed.printOrders.length > 0) {
      inMemory.printOrders = mergeById(parsed.printOrders, inMemory.printOrders);
      notifyPrintOrders();
      loaded = true;
    }
    if (Array.isArray(parsed.verifications) && parsed.verifications.length > 0) {
      inMemory.verifications = mergeById(parsed.verifications, inMemory.verifications);
      notifyVerifications();
      loaded = true;
    }
    if (parsed.settings) {
...
export function initLiveFirestoreListeners(): () => void {
  ...
  try {
    const unsubRegs = subscribeCollectionDocs<MotorcycleRegistration>(
      ...
      (docs) => {
        if (Array.isArray(docs)) {
          inMemory.registrations = mergeById(docs, inMemory.registrations);
          notifyRegistrations();
      ...
      (err) => {
        if (!String(err?.message).includes('Quota limit exceeded')) {
    ...
    const unsubOffs = subscribeCollectionDocs<OfficerAssignment>(
      ...
      (docs) => {
        if (Array.isArray(docs)) {
          inMemory.officers = mergeById(docs, inMemory.officers);
          notifyOfficers();
          lastSyncTime = new Date();
          isCloudConnected = true;
          setGlobalFirestoreError(null);
    ...
    const unsubOrders = subscribeCollectionDocs<PrintBatchOrder>(
      ...
      (docs) => {
        if (Array.isArray(docs)) {
          inMemory.printOrders = mergeById(docs, inMemory.printOrders);
          notifyPrintOrders();
          lastSyncTime = new Date();
          isCloudConnected = true;
          setGlobalFirestoreError(null);
    ...
    const unsubLogs = subscribeCollectionDocs<VerificationLog>(
      ...
      (docs) => {
        if (Array.isArray(docs)) {
          inMemory.verifications = mergeById(docs, inMemory.verifications);
          notifyVerifications();
          lastSyncTime = new Date();
          isCloudConnected = true;
          setGlobalFirestoreError(null);
