...
export const MultiStepRegistrationForm: React.FC<MultiStepRegistrationFormProps> = ({
  ...
  const handleGoToStep3 = () => {
    ...
    if (!drivingPermitPhoto) {
      setValidationError(isAmharic ? 'እባክዎ የፖሊስ ፈቃድ (Police Permit) ሰነድ አታች ያድርጉ!' : 'Please upload Police Permit document!');
      return;
    }
    setValidationError('');
    setCurrentStep(3);
        ...
        <>
          ...
          <div className="grid grid-cols-3 gap-2 !mt-1.5 !mb-1.5">
            ...
            >
              ...
              <span className="truncate">{isAmharic ? 'የሞተርና ባለቤት መረጃ' : 'Text Details'}</span>
            </button>
        ...
        {currentStep === 1 && (
          <div className="space-y-4 animate-fadeIn">
            ...
            <div className="pt-3 border-t border-outline-variant flex justify-end">
              ...
              >
                <span>{isAmharic ? 'ቀጣይ፡ የፖሊስ ፈቃድ እና ሰነዶች አታች ያድርጉ' : 'Next: Attach Police Permit & Documents'}</span>
          ...
          <div className="space-y-4 animate-fadeIn">
            <div className="flex items-center justify-between border-b border-outline-variant pb-2">
              <h4 className="text-xs font-bold text-secondary uppercase tracking-wider flex items-center gap-1.5">
                ...
                <span>{isAmharic ? 'የባለቤት ፎቶ፣ መታወቂያ እና የፖሊስ ፈቃድ' : 'Owner Portrait, ID & Police Permit'}</span>
              </h4>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              {/* Police Permit Scan */}
              <DocumentUploadInput
                label={isAmharic ? '5. የፖሊስ ፈቃድ (Police Permit)' : '5. Police Permit'}
          ...
          <div className="space-y-4 animate-fadeIn">
            ...
            <SectionCard className="space-y-3">
              ...
              <div className="pt-2 border-t border-outline-variant/60">
                <span className="text-secondary text-[11px] block mb-2 font-bold">{isAmharic ? 'የተያያዙ ሰነዶች (የፖሊስ ፈቃድን ጨምሮ):' : 'Attached Documents (Including Police Permit):'}</span>
                <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 text-[11px]">
                  ...
                  <div className="bg-surface p-2 rounded-lg border border-outline-variant flex items-center gap-2">
                    {drivingPermitPhoto ? (
                      <img src={drivingPermitPhoto} alt="Police Permit" className="w-8 h-8 rounded object-cover" />
                    ) : (
                      <span className="material-symbols-outlined text-secondary text-[20px]">menu_book</span>
                    )}
                    <span className="font-bold text-primary truncate">{isAmharic ? 'የፖሊስ ፈቃድ' : 'Police Permit'}</span>
                  </div>
                </div>
              </div>
            </SectionCard>
              ...
              <label className="flex items-start gap-3 cursor-pointer">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-[11px] text-amber-900 dark:text-amber-300 leading-relaxed">
                    {isAmharic
                      ? 'እኔ ከላይ ስሜ የተጠቀሰው መዝጋቢ/ባለቤት፣ ያስገባሁት መረጃ እና ያያያዝኩት የፖሊስ ፈቃድ (Police Permit) ሰነድ ሙሉ በሙሉ እውነተኛና ህጋዊ መሆኑን አረጋግጣለሁ። ሐሰተኛ መረጃ ወይም የተጭበረበረ የፖሊስ ፈቃድ ማቅረብ በህግ ያስጠይቃል።'
                      : 'I hereby confirm that all entered details and attached Police Permit document are accurate, genuine, and legally valid. Providing false information or forged Police Permit documentation is strictly punishable by law.'}
                  </p>
                </div>
              </label>
            </div>
