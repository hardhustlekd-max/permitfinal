...
export const PermitStatusSummary: React.FC<PermitStatusSummaryProps> = ({
  ...
  const statusCards = [
    ...
    {
      key: 'rejected',
      label: isAmharic ? 'የተሰረዙ / የተከለከሉ' : 'Denied / Rejected',
      count: rejectedCount,
    ...
    {
      key: 'printed',
      label: isAmharic ? 'የታተሙና የተሰጡ' : 'Printed & Issued',
      count: printedCount + orderedPrintCount,
      percentage: Math.round(((printedCount + orderedPrintCount) / totalCount) * 100),
      icon: 'badge',
      badgeBg: 'bg-blue-100 text-blue-900 border-blue-200',
      iconBg: 'bg-blue-600 text-white',
      hoverBorder: 'hover:border-blue-400',
      description: isAmharic ? 'ባጅና ስቲከር የታተሙላቸው' : 'Badges produced/dispatched',
    },
  ];
  ...
  return (
    <div id="permit-status-summary" className="bg-surface-container-lowest border border-outline-variant rounded-2xl p-3.5 sm:p-4 shadow-xs space-y-3">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-outline-variant pb-2.5">
        ...
        {/* Total Badge Pill */}
        <div className="flex items-center gap-1.5 px-3 py-1 bg-surface-container border border-outline-variant rounded-xl text-xs font-bold text-on-surface">
          <span className="text-secondary">{isAmharic ? 'ጠቅላላ ማመልከቻዎች:' : 'Total Applications:'}</span>
          <span className="font-mono font-extrabold text-primary text-sm">{registrations.length}</span>
        </div>
      </div>
        ...
        {statusCards.map((card) => (
          ...
          >
            ...
            <div>
              ...
              <p className="text-[10px] text-secondary line-clamp-1 mt-0.5">{card.description}</p>
            </div>
