...
export const SharedScannerModal: React.FC<SharedScannerModalProps> = ({
  ...
  // Force browser fullscreen mode when scan page is active to hide top and bottom browser components
  useEffect(() => {
    if (isOpen) {
      try {
  ...
  const mainCardContent = (
    <>
      ...
      {!scannedRegResult ? (
            ...
            {isScanning ? (
              <div className="absolute inset-0 w-full h-full flex items-center justify-center bg-black overflow-hidden">
                ...
                <div className="absolute inset-0 pointer-events-none flex flex-col justify-between z-10">
                  {/* 1. TOP HEADER (Chevron back arrow + "Scan QR code" title) */}
                  ...
                  {/* 2. CENTER VIEWFINDER (Reversed back to blue corner reticle & red laser line) */}
        ...
        <div className="bg-surface-container-lowest p-4 sm:p-6 m-0 rounded-none border-0 text-xs sm:text-sm shadow-none space-y-3 animate-in fade-in duration-150 flex flex-col h-full max-h-full overflow-hidden justify-center">
        ...
        <div className="bg-surface-container-lowest p-0 m-0 rounded-none border-0 text-xs sm:text-sm flex flex-col h-full max-h-full overflow-hidden animate-in fade-in duration-150">
          {(() => {
            ...
            return (
              <>
                ...
                <div className="flex-1 overflow-y-auto min-h-0 p-3.5 sm:p-4 space-y-3">
                  ...
                  {/* Upper Profile Section with Portrait Photo & Owner Name (Always Visible) */}
                        ...
                        >
                          ...
                          <div className="space-y-2 pt-4 border-t border-outline-variant/40 mt-4">
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-2 pt-1">
                              ...
                              <div
                                onClick={() => setZoomedImage({
                                  url: scannedRegResult.userPortraitPhoto || scannedRegResult.nationalIdPhoto || '',
                              ...
                              >
                                <div className="relative h-28 sm:h-32 rounded-lg overflow-hidden bg-slate-900 flex items-center justify-center">
                                  <SmartImage
                                    src={scannedRegResult.userPortraitPhoto || scannedRegResult.nationalIdPhoto}
                                    alt="User Portrait"
                                    fallbackIcon="person"
                                    className="w-full h-full object-cover group-hover:scale-105 transition-transform"
                                  />
                          ...
                          {/* 3. SECTION: የዲጂታል መታወቂያ ካርድ (Digital ID Card) - Rendered statically inside Vehicle Specs collapsible */}
                          <div className="space-y-2 pt-4 border-t border-outline-variant/40 mt-4">
                            <h4 className="font-bold text-xs sm:text-sm text-on-surface flex items-center gap-1.5">
  ...
  if (isPage) {
    return (
      <div className="w-full h-full flex flex-col flex-1 min-h-0 overflow-hidden bg-slate-950 p-0 m-0 rounded-none">
        {mainCardContent}
      </div>
    );
  }
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-black/85 backdrop-blur-md flex items-center justify-center p-0 overflow-hidden transition-all duration-200">
      <div className="bg-slate-950 rounded-none p-0 m-0 w-full max-w-full sm:max-w-6xl h-[100dvh] sm:h-[98vh] max-h-[100dvh] shadow-2xl flex flex-col overflow-hidden">
        {mainCardContent}
      </div>
    </div>
  );
