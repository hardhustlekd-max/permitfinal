...
export const A4PermitPaper: React.FC<A4PermitPaperProps> = ({ registration, lang, onClose, autoPrint = false }) => {
  ...
  const name = registration.fullName || 'አበበ በቀለ ደስታ (Abebe Bekele)';
  const phone = registration.phone || '+251 911 234 567';
  const plate = registration.plateNumber || 'AA-03-6213';
  const chassis = registration.engineOrSerialNo || 'MD634KE64G2F39841';
  const motorBrand = registration.motorBrand || 'Lifan';
  const motorModel = registration.motorModel || '2026 EV';
  const category = registration.vehicleCategory === 'electric' ? 'ኢቪ (Electric EV)' : 'ቤንዚን (Gasoline <110cc)';
  const issueDate = registration.registrationDate || new Date().toISOString().split('T')[0];
  const expiryDate = '12/10/2026';
  const registeredBy = registration.registeredBy || 'CLERK-001';
  ...
  return (
    <div className="w-full font-sans flex flex-col items-center select-none py-2">
      {/* Sleek Modal Wrapper Card */}
      <div className="bg-surface-container-lowest border border-outline-variant rounded-3xl p-5 sm:p-6 shadow-2xl w-full max-w-4xl space-y-4 relative overflow-hidden my-auto">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-bold text-sm text-on-surface leading-tight">
                {isAmharic ? 'የA4 መንቀሳቀሻ ፍቃድ ኦፊሴላዊ ሰነድ' : 'Full A4 Size Official Movement Permit Document'}
              </h3>
              <p className="text-[11px] font-mono text-secondary font-medium">
                {isAmharic ? 'ፈቃድ ቁጥር፡' : 'Permit No:'} <span className="text-primary font-bold">{regId}</span>
              </p>
            </div>
          </div>
        ...
        {/* PRINTABLE A4 CONTAINER (Standard A4 Proportions: ~794px x 1123px or 210mm x 297mm) */}
        <div className="flex justify-center py-2 overflow-auto max-h-[70vh]">
          <div className="printable-a4-document shrink-0 w-[794px] min-h-[1050px] bg-white text-black p-8 shadow-md rounded-sm border-4 border-double border-black relative font-sans text-xs leading-normal select-text overflow-hidden">
            ...
            <div className="relative z-20 space-y-3">
              ...
              <div className="text-center space-y-0.5 border-b-2 border-black pb-2">
                <div className="flex justify-center items-center gap-2 mb-1">
                  <div className="w-12 h-12 rounded-full border-2 border-black flex items-center justify-center bg-white p-0.5 shadow-xs">
                    <div className="w-full h-full rounded-full bg-black flex items-center justify-center relative overflow-hidden">
                      <svg className="w-8 h-8 text-white" viewBox="0 0 40 40" fill="none">
                        ...
                        <path d="M10 22 C14 14, 22 10, 28 8" stroke="#ffffff" strokeWidth="3" strokeLinecap="round" />
                        <circle cx="28" cy="8" r="2.5" fill="#ffffff" />
                      </svg>
                    </div>
                  </div>
                </div>
                <h1 className="text-xs font-black text-black uppercase tracking-wider">
                  የባህር ዳር እና አካባቢው የሞተር ሳይክሎች መረጃ ማኔጅመንት ባለስልጣን
                </h1>
                <h2 className="text-[11px] font-bold text-black tracking-wide">
                  BAHIRDAR & SURROUNDINGS MOTORCYCLE INFORMATION MANAGEMENT AUTHORITY
                </h2>
                <div className="text-[11px] font-bold text-black pt-0.5">
                  የተሽከርካሪ ምዝገባ እና ፈቃድ ክፍል (RTO BAHIRDAR)
                </div>
              </div>
              ...
              <div className="flex justify-between items-center bg-slate-50 border border-black p-2 my-2">
                <div className="flex items-center gap-3">
                  <div className="w-14 h-18 bg-slate-200 border border-black overflow-hidden flex items-center justify-center shrink-0">
                    {registration.userPortraitPhoto ? (
                      <img src={registration.userPortraitPhoto} alt="Holder" className="w-full h-full object-cover grayscale" crossOrigin="anonymous" referrerPolicy="no-referrer" />
                    ) : (
