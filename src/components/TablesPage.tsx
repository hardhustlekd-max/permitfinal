...
export const TablesPage: React.FC<TablesPageProps> = ({
  ...
  return (
    <div className="bg-surface-container-lowest border border-outline-variant rounded-2xl shadow-xs overflow-hidden">
          ...
          <div className="hidden md:block overflow-x-auto">
            <table className="w-full text-left text-xs">
              <thead className="bg-transparent text-secondary font-bold uppercase text-[10px] tracking-wider border-b border-outline-variant">
                <tr>
                  <th className="px-4 py-3">{isAmharic ? 'ባለቤት / ስም' : 'Owner Name'}</th>
                  <th className="px-4 py-3">{isAmharic ? 'ሰሌዳ ቁጥር' : 'Plate No'}</th>
                  <th className="px-4 py-3">{isAmharic ? 'ዓይነት' : 'Category'}</th>
                  <th className="px-4 py-3">{isAmharic ? 'ሴሪያል ቁጥር' : 'Serial No'}</th>
                  <th className="px-4 py-3">{isAmharic ? 'የቀን ሁኔታ' : 'Reg Date'}</th>
                ...
                {registrations.length === 0 ? (
                  ...
                  filteredRegistrations.map((reg) => (
                    <tr key={reg.id} className="hover:bg-surface-container-low transition-colors text-[13px]">
                      ...
                      <td className="px-4 py-3 font-mono font-medium text-on-surface">{reg.plateNumber}</td>
                      <td className="px-4 py-3">
                        <span className="px-2 py-1 rounded-md border border-outline-variant text-[10px] font-medium text-secondary">
                          {reg.vehicleCategory === 'electric' ? 'Ev' : 'Gasoline'}
                        </span>
                      </td>
                      <td className="px-4 py-3 font-mono text-[12px] text-secondary">{reg.engineOrSerialNo}</td>
                      <td className="px-4 py-3 text-[12px] text-secondary">{reg.registrationDate}</td>
                      <td className="px-4 py-3">
                        <span
                      ...
                      <td className="px-2.5 py-2 text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          {/* Full A4 Size Permit Paper button */}
                          <button
                            type="button"
                            onClick={() => setSelectedRegForA4(reg)}
      ...
      {selectedRegForA4 && (
        <div className="fixed inset-0 z-50 bg-black/75 backdrop-blur-md flex items-start justify-center p-2 sm:p-6 overflow-y-auto transition-all duration-200">
