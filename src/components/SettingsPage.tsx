      ...
      <form onSubmit={handleSaveSettings} className="space-y-4">
        ...
        <div className="bg-surface-container-lowest border border-outline-variant p-3.5 sm:p-4 rounded-2xl shadow-xs space-y-3">
          <div className="flex items-center gap-2 border-b border-outline-variant pb-3">
            ...
            <h3 className="font-bold text-base text-on-surface">
              {isAmharic ? '1. የተጠቃሚ መለያ እና መገለጫ' : '1. User Profile & Official Credentials'}
            </h3>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-secondary mb-1">
                {isAmharic ? 'የባጅ መለያ ቁጥር:' : 'Official Badge ID:'}
              </label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-xs font-bold text-secondary mb-1">
                {isAmharic ? 'የስራ ድርሻ:' : 'Assigned Role:'}
              </label>
              <input
                type="text"
                value={(userRole || '').toUpperCase().replace('_', ' ')}
                disabled
                className="w-full bg-primary/10 border border-primary/20 px-3.5 py-2.5 rounded-xl text-xs font-bold text-primary uppercase cursor-not-allowed"
              />
            </div>
        ...
        <div className="bg-surface-container-lowest border border-outline-variant p-3.5 sm:p-4 rounded-2xl shadow-xs space-y-3">
          <div className="flex items-center gap-2 border-b border-outline-variant pb-3">
            ...
            <h3 className="font-bold text-base text-on-surface">
              {isAmharic ? '2. የካርድ ህትመት እና ማሽን ቅንብር' : '2. Permit Card Printer & Hardware Setup'}
            </h3>
          </div>
            ...
            <div>
              ...
              <div className="relative">
                <select
                  ...
                  style={{ WebkitAppearance: 'none', MozAppearance: 'none', appearance: 'none', background: 'transparent' }} className="w-full bg-surface px-3.5 py-2.5 pr-9 rounded-xl border border-outline-variant focus:border-primary text-xs font-semibold text-on-surface outline-hidden appearance-none cursor-pointer"
                >
                  <option value="">{isAmharic ? 'ምረጥ' : 'ምረጥ'}</option>
                ...
                <Icon name="keyboard_arrow_down" size={18} className="absolute right-3 top-1/2 -translate-y-1/2 text-secondary pointer-events-none" />
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            {onToggleTheme && (
              <div className="sm:col-span-2 flex items-center justify-between p-3.5 bg-surface-container/50 border border-outline-variant rounded-xl">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs font-bold text-on-surface">
                      {isAmharic ? 'የገጽታ ቀለም' : 'Dark Mode UI Theme'}
                    </p>
                    <p className="text-[11px] text-secondary">
                      {isAmharic
            ...
            {onToggleLang && (
              <div>
                ...
                >
                  <span>{isAmharic ? 'አማርኛ' : 'English (United States)'}</span>
        ...
        <div className="bg-surface-container-lowest border border-outline-variant p-6 rounded-2xl shadow-xs space-y-4">
          <div className="flex items-center gap-2 border-b border-outline-variant pb-3">
            ...
            <h3 className="font-bold text-base text-on-surface">
              {isAmharic ? '4. የደህንነት እና ሲስተም ጥገና' : '4. Security Controls & Data Diagnostics'}
            </h3>
          </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between p-3.5 bg-surface-container/50 border border-outline-variant rounded-xl">
              <div>
                <p className="text-xs font-bold text-on-surface">
                  {isAmharic ? 'ሁለት ደረጃ ደህንነት ማረጋገጫ' : 'Two-Factor Authentication (2FA)'}
                </p>
                <p className="text-[11px] text-secondary">
          ...
          <div className="pt-2 flex flex-wrap items-center justify-between gap-3 border-t border-outline-variant">
            ...
            >
              ...
              <span>{isAmharic ? 'የመረጃ ኮፒ ማውረጃ' : 'Export System Data Backup'}</span>
            </button>
